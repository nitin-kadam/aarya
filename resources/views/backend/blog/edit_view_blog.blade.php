          ...
          <div class="">
            ...
            <div class="clearfix"></div>           
             ...
             <div class="x_panel">
                  ...
                  <div class="x_content">
                    <form id="demo-form" action="{{ url('edit_action')}}" method="POST" enctype="multipart/form-data"> 
                      <label for="Heading">Heading * :</label>
                      <input type="text" id="heading" class="form-control" name="heading" value="{{ $blog->heading }}" autocomplete="off" />
                      ...
                      <input type="text" id="subheading" class="form-control" name="subheading" value="{{ $blog->subheading }}" autocomplete="off" />
                      @error('subheading')
                      <div class="alert alert-danger" id="error">{{ $message }}</div>
                      @enderror

                      <label for="short_desc">Short Description * :</label>
                      <textarea rows="3" class="form-control" name="short_desc" id="editor" value="{{ $blog->short_desc}}">{{ $blog->short_desc}}</textarea>
                      @error('short_desc')
                      <div class="alert alert-danger" id="short_desc">{{ $message }}</div>
                      @enderror          

                      <label for="description">Description * :</label>
                      <textarea rows="3" class="form-control" name="description" id="editor1" value="{{ $blog->description}}">{{ $blog->description}}</textarea>
                      @error('description')
                      <div class="alert alert-danger" id="email_error">{{ $message }}</div>
                      @enderror                        
                      
                      <label for="blog_image">Blog Image * :</label>
                      <input type="file" id="blog_image" class="form-control" name="blog_image" value="{{ $blog->image }}"><br>                   
